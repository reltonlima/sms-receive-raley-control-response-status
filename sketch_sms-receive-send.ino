...
void setup() {
  ...
  sim800lSerial->begin(115200, SERIAL_8N1, MODEM_RX, MODEM_TX);
...
void switchCase(int smsIndexSlot, String content){
  if(content == "on"){
        SendSMS(smsIndexSlot,"Ralay foo acionado com sucesso!");
  } else if(content == "off"){
        SendSMS(smsIndexSlot,"Relay desligado com sucesso!");
  } else if(content == "status") {
        SendSMS(smsIndexSlot,"O status do relay eh...");
  } else if(content == "help"){
        SendSMS(smsIndexSlot,"Para usar o serviço, os seguintes comandos podem ser executados: \n [1] on \n [2] off \n [3] status \n [4] help");
  } else {
        SendSMS(smsIndexSlot,"Comando desconhecido. Comando aceitos:  \n [1] on \n [2] off \n [3] status \n [4] help");
  }
...
void SendSMS(int smsIndexSlot, String message) {
  ...
  if (!sim800l.getSMSSender(smsIndexSlot, callerIDbuffer, 31)) {
        Serial.println("Não foi encontrada a mensagem SMS no slot!");
  }
